// From https://github.com/douglasmakey/isoserver/blob/main/src/net.rs
...
pub async fn prepare_net(
    ...
    // println!("Interact with bridge {bridge_name} at cidr {bridge_ip}/{subnet}");

    // create bridge if not exist
    let bridge_idx = match get_bridge_idx(&handle, bridge_name.clone()).await {
        Ok(idx) => {
            // println!("bridge {} already exist", bridge_name);
            idx
        }
...
pub async fn setup_veth_peer(
    ...
    // println!("setup veth peer with ip: {}/{}", ns_ip, subnet);
